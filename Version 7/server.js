...
const generateGiftCardImage = async (recipient, message, amount, cardNumber, barcodeBuffer) => {
    ...
    const html = `
        <div style="width: 300px; height: 550px; background: url('file://${backgroundImagePath}') no-repeat center center/cover; color: white; display: flex; flex-direction: column; justify-content: flex-start; align-items: center; border-radius: 20px; padding-top: 20px;">
            <p style="font-size: 20px; margin: 10px;">${recipient}</p>
            <p style="font-size: 16px; margin: 10px;">${message}</p>
            <p style="font-size: 16px; margin: 10px;">Amount: ${amount} RSD</p>
            ${barcodeBuffer ? `<img src="data:image/png;base64,${barcodeBuffer.toString('base64')}" style="width: 200px; margin-top: 20px;" />` : ''}
        </div>
    `;
    ...
    return htmlToImage({
        ...
        quality: 100,
        type: 'png',
        puppeteer: puppeteer // Pass puppeteer directly
    });
};
    ...
    try {
        ...
        bwipjs.toBuffer({
            ...
            const newGiftCard = new GiftCard({
                ...
                giftCardImage: giftCardImageBuffer
            });
